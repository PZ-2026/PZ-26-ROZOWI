...
import pl.edu.ur.blokur.config.SecurityConfig;
...
/**
 * Testy jednostkowe kontrolera {@link FinancialTransactionController}.
 * Weryfikują obsługę żądań HTTP, autoryzację ról oraz walidację danych
 * wejściowych na poziomie warstwy webowej.
 */
...
class FinancialTransactionControllerTest {
    ...
    @MockitoBean
    private pl.edu.ur.blokur.security.JwtAuthenticationFilter jwtAuthenticationFilter;
    ...
    void setUp() {
        ...
        transactionResponse = new FinancialTransactionResponse(
            UUID.randomUUID(),
            apartmentId,
            "WPLATA",
            new BigDecimal("500.00"),
        ...
        apartmentTransactionsResponse = new ApartmentTransactionsResponse(
        ...
        try {
            org.mockito.stubbing.Answer<Void> doFilterAnswer = invocation -> {
            ...
            org.mockito.Mockito.doAnswer(doFilterAnswer).when(jwtAuthenticationFilter)
    ...
    class GetTransactionsTests {
        ...
        void shouldReturn404WhenApartmentNotFound() throws Exception {
            when(financialTransactionService.getTransactionsForApartment(apartmentId))
                .thenThrow(new NotFoundException("Lokal o ID " + apartmentId + " nie istnieje"));
    ...
    class CreateTransactionTests {
        ...
        void shouldReturn201ForValidRequest() throws Exception {
            ...
            when(financialTransactionService.createTransaction(
                eq(apartmentId), any(FinancialTransactionRequest.class),
                eq("mock.zarzadca1@blokur.pl")
            )).thenReturn(transactionResponse);

            mockMvc.perform(post("/api/apartments/{id}/transactions", apartmentId)
                    .principal(principal)
        ...
        void shouldReturn400WhenTypeMissing() throws Exception {
            FinancialTransactionRequest invalid = new FinancialTransactionRequest(
                null,
                new BigDecimal("500.00"),
                "Opis",
                LocalDate.of(2026, 4, 15)
