    ...
    @Query(
            value =
                    "SELECT t.id, t.ticket_number, t.title, t.status, tc.name AS category_name,"
                        ...
                        + " :buildingId)) AND (:dateFrom IS NULL OR t.created_at >= :dateFrom) AND"
                        + " (:dateTo IS NULL OR t.created_at <= :dateTo) AND (:search IS NULL OR"
    ...
    @Query(
            value =
                    "SELECT t.id, t.ticket_number, t.title, t.status, tc.name AS category_name,"
                        ...
                        + " t.category_id = :categoryId) AND (:dateFrom IS NULL OR t.created_at >="
                        + " :dateFrom) AND (:dateTo IS NULL OR t.created_at <= :dateTo) AND"
    ...
    @Query(
            value =
                    "SELECT t.id, t.ticket_number, t.title, t.status, tc.name AS category_name,"
                        ...
                        + " t.closed_at FROM tickets t JOIN ticket_categories tc ON t.category_id ="
                        + " tc.id JOIN users a ON t.author_id = a.id JOIN users ass ON"
                        + " t.assigned_to_id = ass.id LEFT JOIN apartments ap ON t.apartment_id ="
                        ...
                        + " (:categoryId IS NULL OR t.category_id = :categoryId) AND (:dateFrom IS"
                        + " NULL OR t.created_at >= :dateFrom) AND (:dateTo IS NULL OR t.created_at"
