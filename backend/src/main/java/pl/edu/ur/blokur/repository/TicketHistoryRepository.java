...
/**
 * Repozytorium JPA dla encji {@link TicketHistory}.
 * Zawiera złożone kwerendy łączące tabele ticket_history i users
 * w celu śledzenia audytu zmian statusów zgłoszeń.
 */
@Repository
public interface TicketHistoryRepository extends JpaRepository<TicketHistory, UUID> {

    /**
     * Pobiera pełną historię zmian statusów zgłoszenia wraz z danymi
     * użytkownika dokonującego zmiany.
     * Złącza tabele ticket_history i users.
     *
     * @param ticketId identyfikator zgłoszenia
     * @return lista wpisów historii posortowana od najstarszego
     */
    @Query("SELECT new pl.edu.ur.blokur.dto.TicketHistoryDto("
    ...
    /**
     * Pobiera ostatni wpis historii dla danego zgłoszenia.
     * Używane do wyświetlania aktualnego statusu z informacją kto go ustawił.
     *
     * @param ticketId identyfikator zgłoszenia
    ...
    /**
     * Pobiera historię zmian statusów dla wielu zgłoszeń jednocześnie.
     * Używane do wsadowego ładowania audytu dla listy zgłoszeń.
     *
    ...
    /**
     * Pobiera historię zmian statusów wykonanych przez danego użytkownika.
     * Używane w audycie działań konserwatora lub zarządcy.
     *
