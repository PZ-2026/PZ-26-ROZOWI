...
public class TicketService {
        ...
        public List<TicketSummaryDto> getAll(String username, TicketFilterParams filters) {
                ...
                UUID staircaseId = (apt != null && apt.getStaircase() != null) ? apt.getStaircase().getId() : null;
                UUID buildingId = (apt != null
        ...
        public TicketDetailDto getById(UUID ticketId, String username) {
                ...
                UUID residentStaircaseId = (apt != null && apt.getStaircase() != null) ? apt.getStaircase().getId()
                                : null;
                UUID residentBuildingId = (apt != null
                                && apt.getStaircase() != null
                                && apt.getStaircase().getBuilding() != null)
                                                ? apt.getStaircase().getBuilding().getId()
                                                : null;

                boolean hasAccess = isTicketVisibleForResident(
        ...
        /**
         * Przypisuje zgłoszenie do konserwatora, ustawiając planowaną datę wizyty i
         * notatkę. Operacja
        ...
        public TicketDetailDto assignTicket(
                ...
                User conservator = userRepository
        ...
        /**
         * Zamyka zgłoszenie będące w stanie ZAKONCZONE_DO_WERYFIKACJI. Generuje
         * protokół PDF i zapisuje
        ...
        public TicketDetailDto closeTicket(UUID ticketId, String username) {
                ...
                List<String> beforeImages = ticket.getImages().stream()
                                .filter(
                                                img -> img.getImageType() == pl.edu.ur.blokur.models.TicketImageType.BEFORE)
                                .map(pl.edu.ur.blokur.models.TicketImage::getFilePath)
                                .toList();
                List<String> afterImages = ticket.getImages().stream()
                                .filter(
                                                img -> img.getImageType() == pl.edu.ur.blokur.models.TicketImageType.AFTER)
                                .map(pl.edu.ur.blokur.models.TicketImage::getFilePath)
                                .toList();
        ...
        private TicketSummaryDto mapRawToSummary(Object[] row) {
                ...
                if (slaHours != null && dto.getCreatedAt() != null) {
                        double elapsed = businessHoursCalculator.calculate(dto.getCreatedAt(), LocalDateTime.now());
